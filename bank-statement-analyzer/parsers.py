...
def amex_csv_parser(file):
    ...
    df["Debit"] = (
        df["Amount"].astype(float).apply(lambda x: abs(x) if x < 0 else np.NaN)
    )
    df["Credit"] = (
        df["Amount"].astype(float).apply(lambda x: abs(x) if x > 0 else np.NaN)
    )
...
def capitalone_csv_parser(file):
    df = (
        pd.read_csv(file)
        .drop(columns=["Posted Date", "Card No.", "Category"])
...
def chase_csv_parser(file):
    df = (
        pd.read_csv(file)
        .drop(columns=["Post Date", "Category", "Type", "Memo"])
        .rename(columns={"Transaction Date": "Date"})
    )
    df["Debit"] = (
        df["Amount"].astype(float).apply(lambda x: abs(x) if x < 0 else np.NaN)
...
def discover_csv_parser(file):
    df = (
        pd.read_csv(file)
        .drop(columns=["Post Date", "Category"])
        .rename(columns={"Trans. Date": "Date"})
    )
    df["Debit"] = (
        df["Amount"].astype(float).apply(lambda x: abs(x) if x < 0 else np.NaN)
    )
...
def nordstrom_csv_parser(file):
    df = (
        ...
        .drop(columns=["Posting Date", "Ref#"])
        .rename(
            columns={"Transaction Date": "Date", "Transaction Detail": "Description"}
        )
    )
...
def schwab_csv_parser(file):
    df = (
        pd.read_csv(file)
        .drop(columns=["Status", "Type", "CheckNumber", "RunningBalance"])
        .rename(columns={"Withdrawal": "Credit", "Deposit": "Debit"})
    )
